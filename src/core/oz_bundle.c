#include "../../include/oz/oz_bundle.h"

#include <stdio.h>
#include <stdlib.h>
#include <string.h>

static char *dup_cstr_len(const char *s, size_t n) {
...
bool oz_bundle_load(const char *path, OzBundle *out) {
  ...
  OzBundleEntry *entries =
      (OzBundleEntry *)calloc(count, sizeof(OzBundleEntry));
  if (!entries) {
    fclose(f);
    return false;
  }
