#include "../../include/oz/render/oz_bsp.h"
...
void oz_world_props_default(OzWorldProps *p) {
  if (!p)
    return;
  p->ambient_rgb[0] = 0.15f;
  p->ambient_rgb[1] = 0.15f;
  p->ambient_rgb[2] = 0.18f;
  p->gravity_z = -9.8f;
}
...
OzBrush *oz_map_add_box(OzMap *map, OzVec3 center, OzVec3 size) {
  if (!map)
    return NULL;
  if (!oz_map_reserve(map, map->count + 1))
    return NULL;
...
OzBrush *oz_map_add_cylinder(OzMap *map, OzVec3 center, float radius_x,
                             float radius_y, float height, int segments) {
  if (!map)
    return NULL;
  if (segments < 3)
    segments = 3;
...
OzBrush *oz_map_add_sphere(OzMap *map, OzVec3 center, float radius,
                           int segments) {
  if (!map)
    return NULL;
  if (segments < 6)
...
OzBrush *oz_map_add_pyramid(OzMap *map, OzVec3 center, float half_x,
                            float half_y, float height) {
  if (!map)
    return NULL;
  if (!oz_map_reserve(map, map->count + 1))
    return NULL;
  ...
  b->as.pyr.center = center;
  b->as.pyr.half_x = half_x;
  b->as.pyr.half_y = half_y;
  b->as.pyr.height = height;
  return b;
}

OzBrush *oz_map_add_plane(OzMap *map, OzVec3 center, OzVec3 normal,
                          float half_extent) {
  if (!map)
    return NULL;
  if (!oz_map_reserve(map, map->count + 1))
  ...
  b->as.plane.center = center;
  b->as.plane.normal = normal;
  b->as.plane.half_extent = half_extent;
  return b;
}
  ...
  for (size_t i = 0; ok && i < map->count; ++i) {
    ...
    if (b->type == OZ_BRUSH_BOX) {
      ...
      if (fprintf(f, "box %g %g %g %g %g %g %g\n", (double)bx->center.x,
                  (double)bx->center.y, (double)bx->center.z,
                  (double)(bx->half.x * 2.0f), (double)(bx->half.y * 2.0f),
                  (double)(bx->half.z * 2.0f), (double)bx->rotation_z) < 0)
        ok = 0;
    } else if (b->type == OZ_BRUSH_CYLINDER) {
      const OzBrushCylinder *cy = &b->as.cyl;
      if (fprintf(f, "cyl %g %g %g %g %g %g %d %g\n", (double)cy->center.x,
                  (double)cy->center.y, (double)cy->center.z,
                  (double)cy->radius_x, (double)cy->radius_y,
                  (double)cy->height, cy->segments, (double)cy->rotation_z) < 0)
        ok = 0;
    } else if (b->type == OZ_BRUSH_SPHERE) {
      const OzBrushSphere *sp = &b->as.sph;
      if (fprintf(f, "sph %g %g %g %g %d\n", (double)sp->center.x,
                  (double)sp->center.y, (double)sp->center.z,
                  (double)sp->radius, sp->segments) < 0)
        ok = 0;
    } else if (b->type == OZ_BRUSH_PYRAMID) {
      const OzBrushPyramid *py = &b->as.pyr;
      if (fprintf(f, "pyr %g %g %g %g %g %g\n", (double)py->center.x,
                  (double)py->center.y, (double)py->center.z,
                  (double)py->half_x * 2.0, (double)py->half_y * 2.0,
                  (double)py->height) < 0)
        ok = 0;
    } else if (b->type == OZ_BRUSH_PLANE) {
      const OzBrushPlane *pl = &b->as.plane;
      if (fprintf(f, "pln %g %g %g %g %g %g %g\n", (double)pl->center.x,
                  (double)pl->center.y, (double)pl->center.z,
                  (double)pl->normal.x, (double)pl->normal.y,
                  (double)pl->normal.z, (double)pl->half_extent) < 0)
        ok = 0;
    }
  }
  if (fclose(f) != 0)
...
bool oz_map_load_text(const char *path, OzMap *out_map) {
  ...
  if (strncmp(header, "OZONE 1", 7) != 0 &&
      strncmp(header, "OZMAP 1", 7) != 0) {
    fclose(f);
    return false;
  }
  ...
  if (!oz_map_reserve(out_map, expected)) {
    fclose(f);
    return false;
  }
  ...
  for (size_t i = 0; i < expected; ++i) {
    ...
    if (strcmp(kind, "box") == 0) {
      ...
      if (n == 7)
        nb->as.box.rotation_z = rz;
    } else if (strcmp(kind, "cyl") == 0) {
      float cx, cy, cz, rx, ry, h, rz = 0.0f;
      int seg;
      int n;
      ...
      OzBrush *nb =
          oz_map_add_cylinder(out_map, (OzVec3){cx, cy, cz}, rx, ry, h, seg);
      if (!nb) {
        fclose(f);
        oz_map_free(out_map);
        return false;
      ...
      if (!oz_map_add_sphere(out_map, (OzVec3){cx, cy, cz}, r, seg)) {
        fclose(f);
        oz_map_free(out_map);
        return false;
      }
      ...
      if (!oz_map_add_pyramid(out_map, (OzVec3){cx, cy, cz}, sx * 0.5f,
                              sy * 0.5f, h)) {
        fclose(f);
        oz_map_free(out_map);
        return false;
      ...
      if (!oz_map_add_plane(out_map, (OzVec3){cx, cy, cz}, (OzVec3){nx, ny, nz},
                            he)) {
        fclose(f);
        oz_map_free(out_map);
        return false;
...
static void aabb_union(const OzVec3 a_min, const OzVec3 a_max,
                       const OzVec3 b_min, const OzVec3 b_max, OzVec3 *out_min,
                       OzVec3 *out_max) {
...
static bool aabb_intersect(const OzVec3 a_min, const OzVec3 a_max,
                           const OzVec3 b_min, const OzVec3 b_max,
                           OzVec3 *out_min, OzVec3 *out_max) {
  out_min->x = (a_min.x > b_min.x) ? a_min.x : b_min.x;
  out_min->y = (a_min.y > b_min.y) ? a_min.y : b_min.y;
  out_min->z = (a_min.z > b_min.z) ? a_min.z : b_min.z;
  out_max->x = (a_max.x < b_max.x) ? a_max.x : b_max.x;
...
bool oz_csg_intersect(const OzBrush *a, const OzBrush *b, OzBrush *out) {
  ...
  brush_get_aabb(a, &amin, &amax);
  brush_get_aabb(b, &bmin, &bmax);
  if (!aabb_intersect(amin, amax, bmin, bmax, &imin, &imax))
    return false;
  out->type = OZ_BRUSH_BOX;
...
bool oz_csg_subtract(const OzBrush *a, const OzBrush *b, OzBrush *out) {
  if (!a || !b || !out)
    return false;
  // Approximate: subtracting B from A yields intersection of A with complement
  // of B. Using AABB, this devolves to (AABB(A) minus AABB(B)) which is not a
  // single box in general. We return the intersection if it is non-empty;
  // otherwise false. Better: if B's AABB is fully inside A's AABB, shrink along
  // the largest axis.
  ...
  if (axis == 0) {
    ...
    if (bmin.z - amin.z > amax.z - bmax.z)
      nmin.z = bmax.z;
    else
      nmax.z = bmin.z;
  }
...
static void bsp_tree_reset(OzBspTree *t) {
  if (!t)
    return;
  free(t->nodes);
  t->nodes = NULL;
...
static int bsp_alloc_node(OzBspTree *t) {
  ...
  n->d = 0.0f;
  n->left = n->right = -1;
  n->brush_indices = NULL;
  n->brush_count = 0;
  return idx;
}

static void compute_brush_aabb_list(const OzMap *map, const int *indices,
                                    size_t count, OzVec3 *out_min,
                                    OzVec3 *out_max) {
  out_min->x = out_min->y = out_min->z = 1e9f;
  out_max->x = out_max->y = out_max->z = -1e9f;
  for (size_t i = 0; i < count; ++i) {
    OzVec3 mn, mx;
    brush_get_aabb(&map->brushes[indices[i]], &mn, &mx);
    if (mn.x < out_min->x)
      out_min->x = mn.x;
    if (mn.y < out_min->y)
      out_min->y = mn.y;
...
static int bsp_build_recursive(const OzMap *map, OzBspTree *t,
                               const int *indices, size_t count, int depth) {
  ...
  if (count <= 2 || depth > 16) {
    ...
    if (node->brush_indices) {
      memcpy(node->brush_indices, indices, sizeof(int) * count);
      node->brush_count = count;
    }
    return node_idx;
  }
  OzVec3 mn, mx;
  compute_brush_aabb_list(map, indices, count, &mn, &mx);
  ...
  for (size_t i = 0; i < count; ++i) {
    OzVec3 bmn, bmx;
    brush_get_aabb(&map->brushes[indices[i]], &bmn, &bmx);
    float center = (axis == 0 ? (bmn.x + bmx.x) * 0.5f
                              : (axis == 1 ? (bmn.y + bmx.y) * 0.5f
                                           : (bmn.z + bmx.z) * 0.5f));
    if (center < split)
      left[lc++] = indices[i];
    else
...
bool oz_bsp_build_from_map(const OzMap *map, OzBspTree *out_tree) {
  ...
  for (size_t i = 0; i < map->count; ++i)
    indices[i] = (int)i;
  out_tree->nodes = NULL;
  out_tree->count = out_tree->capacity = 0;
  out_tree->root = -1;
