...
static const char* level_to_string(OzLogLevel level) {
    switch (level) {
        case OZ_LOG_TRACE: return "TRACE";
        case OZ_LOG_DEBUG: return "DEBUG";
        case OZ_LOG_INFO:  return "INFO";
        case OZ_LOG_WARN:  return "WARN";
        case OZ_LOG_ERROR: return "ERROR";
        case OZ_LOG_FATAL: return "FATAL";
        default: return "LOG";
    }
}
...
static const char* level_to_color(OzLogLevel level) {
    switch (level) {
        case OZ_LOG_TRACE: return "\x1b[90m"; // bright black
        case OZ_LOG_DEBUG: return "\x1b[36m"; // cyan
        case OZ_LOG_INFO:  return "\x1b[32m"; // green
        case OZ_LOG_WARN:  return "\x1b[33m"; // yellow
        case OZ_LOG_ERROR: return "\x1b[31m"; // red
        case OZ_LOG_FATAL: return "\x1b[35m"; // magenta
        default: return "";
    }
}
...
void oz_log_logv(OzLogLevel level, const char* fmt, va_list args) {
    ...
    char linebuf[1200];
    snprintf(linebuf, sizeof(linebuf), "[%s] %s: %s", timebuf, level_to_string(level), msgbuf);
    ...
    if (g_also_stderr) {
        ...
        if (use_color) fprintf(stderr, "%s", level_to_color(level));
        fprintf(stderr, "%s", linebuf);
        if (use_color) fprintf(stderr, "\x1b[0m");
        fputc('\n', stderr);
    }
}
