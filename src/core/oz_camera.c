...
float oz_camera_update_freemove(OzCamera* cam, float dt_seconds,
    ...
    // Define forward as the camera facing direction on XY: yaw=0 -> +Y, yaw=90° -> +X
    // This makes standard FPS controls intuitive when yaw increases counter-clockwise.
...
float oz_camera_update_fps(OzCamera* cam, float dt_seconds,
    ...
    if (key_down(OZ_KEY_A)) dx -= 1.0f;
    if (key_down(OZ_KEY_D)) dx += 1.0f;
    float len = sqrtf(dx*dx + dy*dy); if (len > 0.0001f) { dx/=len; dy/=len; }
    float c = cosf(cam->yaw), s = sinf(cam->yaw);
    ...
    if (cam->position.z <= 0.0f) {
        cam->position.z = 0.0f; vel_z = 0.0f;
        if (key_down(OZ_KEY_SPACE)) vel_z = 5.0f;
    }
    vel_z += gravity * dt_seconds;
    cam->position.z += vel_z * dt_seconds;
    if (cam->position.z < 0.0f) cam->position.z = 0.0f;
