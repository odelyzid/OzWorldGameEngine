...
static GtkWidget *build_template_list(const char **items, int count,
                                      GCallback on_activate,
                                      gpointer user_data);
...
static void choose_and_remember(EditorUi *ui, const char *title,
                                const char *filter_name, const char *pattern,
                                char **last_path_out) {
  GtkWidget *dialog = gtk_file_chooser_dialog_new(
      title, GTK_WINDOW(ui->window), GTK_FILE_CHOOSER_ACTION_OPEN, "_Cancel",
  ...
  if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
    ...
    if (last_path_out) {
      g_free(*last_path_out);
      *last_path_out = g_strdup(filename);
    }
...
void action_open(GSimpleAction *action, GVariant *parameter,
  ...
  EditorUi *ui = (EditorUi *)user_data;
  GtkWidget *dialog = gtk_file_chooser_dialog_new(
      "Open Map", GTK_WINDOW(ui->window), GTK_FILE_CHOOSER_ACTION_OPEN,
...
void action_save(GSimpleAction *action, GVariant *parameter,
  ...
  EditorUi *ui = (EditorUi *)user_data;
  GtkWidget *dialog = gtk_file_chooser_dialog_new(
      "Save Map", GTK_WINDOW(ui->window), GTK_FILE_CHOOSER_ACTION_SAVE,
  ...
  if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
    char *filename = gtk_file_chooser_get_filename(GTK_FILE_CHOOSER(dialog));
    char *to_save = NULL;
    if (filename && !(g_str_has_suffix(filename, ".ozone") ||
                      g_str_has_suffix(filename, ".ozmap"))) {
...
static void choose_and_remember(EditorUi *ui, const char *title,
                                const char *filter_name, const char *pattern,
                                char **last_path_out);
static void on_import_texture(GSimpleAction *a, GVariant *p,
                              gpointer user_data) {
  (void)a;
  (void)p;
  EditorUi *ui = (EditorUi *)user_data;
  if (!ui)
    return;
  choose_and_remember(ui, "Import Texture", "OzTex files (*.oztex)", "*.oztex",
                      &ui->last_texture_path);
  if (ui->last_texture_path && ui->gl_area) {
    ...
    if (oz_tex_load_oztex(ui->last_texture_path, &w, &h, &c, &px)) {
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, (c == 4 ? GL_RGBA : GL_RGB), w, h, 0, fmt,
                   GL_UNSIGNED_BYTE, px);
      ui->gl_tex_brush = tex;
      ui->gl_tex_w = w;
      ui->gl_tex_h = h;
...
static void on_import_bundle(GSimpleAction *a, GVariant *p,
                             gpointer user_data) {
  (void)a;
  (void)p;
  EditorUi *ui = (EditorUi *)user_data;
  if (!ui)
    return;
  choose_and_remember(ui, "Import Mesh Bundle", "OzBag files (*.ozbag)",
                      "*.ozbag", &ui->last_bundle_path);
...
static void on_import_music(GSimpleAction *a, GVariant *p, gpointer user_data) {
  ...
  if (!ui)
    return;
  choose_and_remember(ui, "Import Music", "OzMux files (*.ozmux)", "*.ozmux",
                      &ui->last_music_path);
}
...
void action_build_map(GSimpleAction *a, GVariant *p, gpointer u) {
  (void)a;
  (void)p;
  (void)u;
  action_build_common("Map");
}
void action_build_light(GSimpleAction *a, GVariant *p, gpointer u) {
  (void)a;
  (void)p;
  (void)u;
  action_build_common("Light");
}
void action_build_bsp(GSimpleAction *a, GVariant *p, gpointer u) {
  (void)a;
  (void)p;
  (void)u;
  action_build_common("BSP");
}
void action_build_brushes(GSimpleAction *a, GVariant *p, gpointer u) {
  (void)a;
  (void)p;
  (void)u;
  action_build_common("Brushes");
}

// --- Lighting add actions ---
static void on_light_add_point(GSimpleAction *a, GVariant *p,
                               gpointer user_data) {
  (void)a;
  (void)p;
  EditorUi *ui = (EditorUi *)user_data;
  GtkWidget *dlg = gtk_dialog_new_with_buttons(
      "Add Point Light", ui && ui->window ? GTK_WINDOW(ui->window) : NULL,
      GTK_DIALOG_MODAL, "_Cancel", GTK_RESPONSE_CANCEL, "_Add",
      GTK_RESPONSE_ACCEPT, NULL);
  GtkWidget *ex = NULL, *ey = NULL, *ez = NULL, *er = NULL, *eg = NULL,
            *eb = NULL, *ei = NULL, *era = NULL;
  dialog_set_entry(GTK_DIALOG(dlg), "X", &ex, "2.0");
  dialog_set_entry(GTK_DIALOG(dlg), "Y", &ey, "2.0");
  dialog_set_entry(GTK_DIALOG(dlg), "Z", &ez, "3.0");
  dialog_set_entry(GTK_DIALOG(dlg), "R", &er, "1.0");
  dialog_set_entry(GTK_DIALOG(dlg), "G", &eg, "0.95");
  dialog_set_entry(GTK_DIALOG(dlg), "B", &eb, "0.9");
  dialog_set_entry(GTK_DIALOG(dlg), "Intensity", &ei, "2.0");
  dialog_set_entry(GTK_DIALOG(dlg), "Radius", &era, "8.0");
  gtk_widget_show_all(dlg);
  if (gtk_dialog_run(GTK_DIALOG(dlg)) == GTK_RESPONSE_ACCEPT) {
    float x = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(ex)), NULL);
    float y = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(ey)), NULL);
    float z = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(ez)), NULL);
    float r = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(er)), NULL);
    float g = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(eg)), NULL);
    float b = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(eb)), NULL);
    float I = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(ei)), NULL);
    float rad = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(era)), NULL);
    (void)oz_lights_add_point(x, y, z, r, g, b, I, rad);
    if (ui && ui->viewport)
      gtk_widget_queue_draw(ui->viewport);
  }
...
static void on_light_add_spot(GSimpleAction *a, GVariant *p,
                              gpointer user_data) {
  (void)a;
  (void)p;
  EditorUi *ui = (EditorUi *)user_data;
  GtkWidget *dlg = gtk_dialog_new_with_buttons(
      "Add Spotlight", ui && ui->window ? GTK_WINDOW(ui->window) : NULL,
      GTK_DIALOG_MODAL, "_Cancel", GTK_RESPONSE_CANCEL, "_Add",
      GTK_RESPONSE_ACCEPT, NULL);
  GtkWidget *ex = NULL, *ey = NULL, *ez = NULL, *edx = NULL, *edy = NULL,
            *edz = NULL, *eang = NULL, *efall = NULL, *er = NULL, *eg = NULL,
            *eb = NULL, *ei = NULL, *era = NULL;
  dialog_set_entry(GTK_DIALOG(dlg), "X", &ex, "-2.0");
  dialog_set_entry(GTK_DIALOG(dlg), "Y", &ey, "-2.0");
  dialog_set_entry(GTK_DIALOG(dlg), "Z", &ez, "3.0");
  dialog_set_entry(GTK_DIALOG(dlg), "Dir X", &edx, "0.6");
  dialog_set_entry(GTK_DIALOG(dlg), "Dir Y", &edy, "0.6");
  dialog_set_entry(GTK_DIALOG(dlg), "Dir Z", &edz, "-1.0");
  dialog_set_entry(GTK_DIALOG(dlg), "Cone Angle (deg)", &eang, "25.0");
  dialog_set_entry(GTK_DIALOG(dlg), "Falloff", &efall, "3.0");
  dialog_set_entry(GTK_DIALOG(dlg), "R", &er, "0.7");
  dialog_set_entry(GTK_DIALOG(dlg), "G", &eg, "0.8");
  dialog_set_entry(GTK_DIALOG(dlg), "B", &eb, "1.0");
  dialog_set_entry(GTK_DIALOG(dlg), "Intensity", &ei, "1.6");
  dialog_set_entry(GTK_DIALOG(dlg), "Radius", &era, "10.0");
  gtk_widget_show_all(dlg);
  if (gtk_dialog_run(GTK_DIALOG(dlg)) == GTK_RESPONSE_ACCEPT) {
    float x = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(ex)), NULL);
    float y = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(ey)), NULL);
    float z = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(ez)), NULL);
    float dx = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(edx)), NULL);
    float dy = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(edy)), NULL);
    float dz = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(edz)), NULL);
    float ang =
        (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(eang)), NULL);
    float fall =
        (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(efall)), NULL);
    float r = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(er)), NULL);
    float g = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(eg)), NULL);
    float b = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(eb)), NULL);
    float I = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(ei)), NULL);
    float rad = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(era)), NULL);
    (void)oz_lights_add_spot(x, y, z, dx, dy, dz, ang, fall, r, g, b, I, rad);
    if (ui && ui->viewport)
      gtk_widget_queue_draw(ui->viewport);
  }
  gtk_widget_destroy(dlg);
...
static void on_light_add_wave(GSimpleAction *a, GVariant *p,
                              gpointer user_data) {
  (void)a;
  (void)p;
  EditorUi *ui = (EditorUi *)user_data;
  GtkWidget *dlg = gtk_dialog_new_with_buttons(
      "Add Wave Light", ui && ui->window ? GTK_WINDOW(ui->window) : NULL,
      GTK_DIALOG_MODAL, "_Cancel", GTK_RESPONSE_CANCEL, "_Add",
      GTK_RESPONSE_ACCEPT, NULL);
  GtkWidget *ex = NULL, *ey = NULL, *ez = NULL, *er = NULL, *eg = NULL,
            *eb = NULL, *ei = NULL, *era = NULL, *ef = NULL, *ea = NULL,
            *ep = NULL;
  dialog_set_entry(GTK_DIALOG(dlg), "X", &ex, "0.0");
  dialog_set_entry(GTK_DIALOG(dlg), "Y", &ey, "0.0");
  dialog_set_entry(GTK_DIALOG(dlg), "Z", &ez, "2.0");
  dialog_set_entry(GTK_DIALOG(dlg), "R", &er, "0.3");
  dialog_set_entry(GTK_DIALOG(dlg), "G", &eg, "0.5");
  dialog_set_entry(GTK_DIALOG(dlg), "B", &eb, "1.0");
  dialog_set_entry(GTK_DIALOG(dlg), "Base Intensity", &ei, "1.2");
  dialog_set_entry(GTK_DIALOG(dlg), "Radius", &era, "12.0");
  dialog_set_entry(GTK_DIALOG(dlg), "Frequency (Hz)", &ef, "0.5");
  dialog_set_entry(GTK_DIALOG(dlg), "Amplitude", &ea, "0.35");
  dialog_set_entry(GTK_DIALOG(dlg), "Phase", &ep, "0.0");
  gtk_widget_show_all(dlg);
  if (gtk_dialog_run(GTK_DIALOG(dlg)) == GTK_RESPONSE_ACCEPT) {
    float x = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(ex)), NULL);
    float y = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(ey)), NULL);
    ...
    float g = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(eg)), NULL);
    float b = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(eb)), NULL);
    float bi = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(ei)), NULL);
    float rad = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(era)), NULL);
    float fq = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(ef)), NULL);
    float amp = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(ea)), NULL);
    float ph = (float)g_ascii_strtod(gtk_entry_get_text(GTK_ENTRY(ep)), NULL);
    (void)oz_lights_add_wave(x, y, z, r, g, b, bi, rad, fq, amp, ph);
    if (ui && ui->viewport)
      gtk_widget_queue_draw(ui->viewport);
  }
  gtk_widget_destroy(dlg);
...
static void spawn_process(char const *const argv[]) {
  ...
  gboolean ok = g_spawn_async(NULL, (gchar **)argv, envp, G_SPAWN_SEARCH_PATH,
...
void action_launch_editor(GSimpleAction *a, GVariant *p, gpointer u) {
  (void)a;
  (void)p;
  (void)u;
  const char *argv[] = {"./build/oz_editor", NULL};
  spawn_process(argv);
}
void action_launch_game(GSimpleAction *a, GVariant *p, gpointer u) {
  (void)a;
  (void)p;
  EditorUi *ui = (EditorUi *)u;
  char arg_pos[128] = {0};
  int found = -1;
  if (ui && ui->obj_count) {
    for (size_t i = 0; i < ui->obj_count; ++i)
      if (ui->objects[i].type == OBJ_PLAYER_START) {
        found = (int)i;
        break;
      }
  }
  const char *argv_default[] = {"./build/oz_demo", NULL};
  char *argv_custom[7] = {(char *)"./build/oz_demo",
                          (char *)"--playerstart",
                          arg_pos,
                          (char *)"--mode",
                          (char *)"freemove",
  ...
  if (found >= 0) {
    ...
    g_snprintf(arg_pos, sizeof(arg_pos), "%g,%g,%g,%g", (double)ps->position[0],
               (double)ps->position[1], (double)ps->position[2],
               (double)ps->yaw);
    const char *mode = "freemove";
    if (ps->camera_mode == OZ_CAMERA_FPS)
      mode = "fps";
    else if (ps->camera_mode == OZ_CAMERA_CINEMATIC)
      mode = "cinematic";
...
void action_launch_server(GSimpleAction *a, GVariant *p, gpointer u) {
  (void)a;
  (void)p;
  (void)u;
  OZ_WARN("Server launch not implemented");
}
...
void action_gizmo_translate(GSimpleAction *a, GVariant *p, gpointer user_data) {
  (void)a;
  (void)p;
  EditorUi *ui = (EditorUi *)user_data;
...
void action_gizmo_rotate(GSimpleAction *a, GVariant *p, gpointer user_data) {
  (void)a;
  (void)p;
  EditorUi *ui = (EditorUi *)user_data;
  ui->gizmo_translate = FALSE;
  ui->gizmo_rotate = TRUE;
  ui->gizmo_scale = FALSE;
}
void action_gizmo_scale(GSimpleAction *a, GVariant *p, gpointer user_data) {
  (void)a;
  (void)p;
  EditorUi *ui = (EditorUi *)user_data;
  ui->gizmo_translate = FALSE;
...
void action_debug_toggle_grid(GSimpleAction *a, GVariant *p,
                              gpointer user_data) {
  (void)a;
  (void)p;
  EditorUi *ui = (EditorUi *)user_data;
  ui->dbg_show_grid = !ui->dbg_show_grid;
  if (ui->viewport)
    gtk_widget_queue_draw(ui->viewport);
}
void action_debug_toggle_axes(GSimpleAction *a, GVariant *p,
                              gpointer user_data) {
  (void)a;
  (void)p;
  EditorUi *ui = (EditorUi *)user_data;
  ui->dbg_show_axes = !ui->dbg_show_axes;
  if (ui->viewport)
    gtk_widget_queue_draw(ui->viewport);
}
void action_debug_toggle_fps(GSimpleAction *a, GVariant *p,
                             gpointer user_data) {
  (void)a;
  (void)p;
  EditorUi *ui = (EditorUi *)user_data;
  ...
  if (!ui->dbg_show_fps)
    gtk_label_set_text(GTK_LABEL(ui->info_label), "");
}

// Log window and debug info actions
typedef struct LogWindowCtx {
  GtkWidget *win;
  GtkWidget *view;
  GtkTextBuffer *buf;
} LogWindowCtx;
...
static void log_sink_bridge(OzLogLevel level, const char *line) {
  (void)level;
  if (!g_log_ctx || !g_log_ctx->buf)
    return;
  GtkTextIter end;
  gtk_text_buffer_get_end_iter(g_log_ctx->buf, &end);
...
void action_debug_open_log(GSimpleAction *a, GVariant *p, gpointer user_data) {
  ...
  (void)p;
  (void)user_data;
  if (g_log_ctx && g_log_ctx->win) {
    gtk_window_present(GTK_WINDOW(g_log_ctx->win));
    return;
  }
...
static gchar *get_memory_info_string(void) {
  ...
  return g_strdup_printf("RSS: %ld KB  MinorFaults: %ld  MajorFaults: %ld",
                         (long)ru.ru_maxrss, (long)ru.ru_minflt,
                         (long)ru.ru_majflt);
#else
...
static void fill_stack_trace(GtkTextBuffer *buf) {
  ...
  for (int i = 0; i < n; ++i) {
    gtk_text_buffer_insert(buf, &end, syms[i], -1);
...
void action_debug_open_debug(GSimpleAction *a, GVariant *p,
                             gpointer user_data) {
  (void)a;
  (void)p;
  EditorUi *ui = (EditorUi *)user_data;
  if (ui->debug_window && GTK_IS_WIDGET(ui->debug_window)) {
    gtk_window_present(GTK_WINDOW(ui->debug_window));
    return;
  }
  ...
  GtkWidget *watch_tv = gtk_text_view_new();
  gtk_text_view_set_editable(GTK_TEXT_VIEW(watch_tv), FALSE);
  gtk_container_add(GTK_CONTAINER(watch_sc), watch_tv);
  GtkTextBuffer *watch_buf = gtk_text_view_get_buffer(GTK_TEXT_VIEW(watch_tv));
  GtkTextIter wend;
  gtk_text_buffer_get_end_iter(watch_buf, &wend);
  ...
  for (size_t i = 0; i < oz_debug_watch_count(); ++i) {
    ...
    case OZ_DEBUG_T_I32:
      ...
      break;
    case OZ_DEBUG_T_U32:
      snprintf(line, sizeof(line), "%s = %u\n", nm, *(const unsigned *)addr);
      break;
    case OZ_DEBUG_T_F32:
      snprintf(line, sizeof(line), "%s = %.3f\n", nm, *(const float *)addr);
      break;
    case OZ_DEBUG_T_F64:
      snprintf(line, sizeof(line), "%s = %.3f\n", nm, *(const double *)addr);
      break;
    default:
  ...
  GtkWidget *asm_tv = gtk_text_view_new();
  gtk_text_view_set_editable(GTK_TEXT_VIEW(asm_tv), FALSE);
  gtk_container_add(GTK_CONTAINER(asm_sc), asm_tv);
  GtkTextBuffer *asm_buf = gtk_text_view_get_buffer(GTK_TEXT_VIEW(asm_tv));
  GtkTextIter aend;
  gtk_text_buffer_get_end_iter(asm_buf, &aend);
#ifndef _WIN32
  void *addrs[64];
  int n = backtrace(addrs, 64);
  if (n >= 2) {
    ...
    snprintf(cmd, sizeof(cmd),
             "sh -lc 'objdump -d --no-show-raw-insn --start-address=0x%lx "
             "--stop-address=0x%lx /proc/self/exe 2>/dev/null || llvm-objdump "
             "-d --no-show-raw-insn --start-address=0x%lx --stop-address=0x%lx "
             "/proc/self/exe 2>/dev/null'",
             (unsigned long)start, (unsigned long)stop, (unsigned long)start,
             (unsigned long)stop);
    FILE *fp = popen(cmd, "r");
    if (fp) {
      char line[512];
      ...
      gtk_text_buffer_insert(asm_buf, &aend,
                             "Failed to run objdump/llvm-objdump.\n", -1);
    }
  } else {
    gtk_text_buffer_insert(asm_buf, &aend,
                           "No stack frames available for disassembly.\n", -1);
  }
#else
  gtk_text_buffer_insert(asm_buf, &aend,
                         "Disassembly not available on this platform.\n", -1);
#endif
  gtk_widget_show_all(win);
  ui->debug_window = win;
}
void action_debug_jit_example(GSimpleAction *a, GVariant *p,
                              gpointer user_data) {
  (void)a;
  (void)p;
  (void)user_data;
  ...
  void *h = NULL;
  if (!oz_jit_compile_and_load("ozjit_tmp", src, "-O2 -fPIC", &h)) {
    OZ_ERROR("JIT compile failed");
    return;
  }
  typedef int (*AddFn)(int, int);
  AddFn fn = (AddFn)oz_jit_get_symbol(h, "ozjit_add");
  if (!fn) {
    OZ_ERROR("JIT: symbol not found");
    oz_jit_unload(h);
    return;
  }
  int r = fn(40, 2);
...
static void place_near_camera(float pos[3]) {
  // Fallback placement near origin; avoids depending on editor input internals
  // here
  pos[0] = 2.0f;
  pos[1] = 0.0f;
  pos[2] = 0.0f;
...
static void object_list_row_activated(GtkListBox *box, GtkListBoxRow *row,
                                      gpointer user_data) {
  (void)box;
  EditorUi *ui = (EditorUi *)user_data;
  if (!ui)
...
static void add_zone_template(GtkListBox *box, GtkListBoxRow *row,
                              gpointer user_data) {
  (void)box;
  (void)row;
  EditorUi *ui = (EditorUi *)user_data;
  ...
  EditorObject o = {0};
  o.type = OBJ_ZONE;
  o.as.zone.name = g_strdup("Zone");
  o.as.zone.radius = 5.0f;
  place_near_camera(o.as.zone.center);
  int idx = objects_add(ui, &o);
  ui->selected_object = idx;
  if (ui->obj_scene_list && GTK_IS_LIST_BOX(ui->obj_scene_list)) {
    char buf[128];
    snprintf(buf, sizeof(buf), "Zone #%d", idx);
    gtk_list_box_insert(GTK_LIST_BOX(ui->obj_scene_list), gtk_label_new(buf),
                        -1);
...
static void add_pickup_template(GtkListBox *box, GtkListBoxRow *row,
                                gpointer user_data) {
  (void)box;
  (void)row;
  EditorUi *ui = (EditorUi *)user_data;
  ...
  EditorObject o = {0};
  o.type = OBJ_PICKUP;
  o.as.pickup.name = g_strdup("Health");
  o.as.pickup.respawn_seconds = 30.0f;
  place_near_camera(o.as.pickup.position);
  int idx = objects_add(ui, &o);
  ui->selected_object = idx;
  if (ui->obj_scene_list && GTK_IS_LIST_BOX(ui->obj_scene_list)) {
    char buf[128];
    snprintf(buf, sizeof(buf), "Pickup #%d", idx);
    gtk_list_box_insert(GTK_LIST_BOX(ui->obj_scene_list), gtk_label_new(buf),
                        -1);
    gtk_widget_show_all(ui->obj_scene_list);
  }
...
static void add_playerstart_template(GtkListBox *box, GtkListBoxRow *row,
                                     gpointer user_data) {
  (void)box;
  (void)row;
  EditorUi *ui = (EditorUi *)user_data;
  if (!ui)
    return;
  EditorObject o = {0};
  o.type = OBJ_PLAYER_START;
  place_near_camera(o.as.pstart.position);
  o.as.pstart.yaw = 0.0f;
  o.as.pstart.camera_mode = OZ_CAMERA_FPS;
  int idx = objects_add(ui, &o);
  ui->selected_object = idx;
  if (ui->obj_scene_list && GTK_IS_LIST_BOX(ui->obj_scene_list)) {
    char buf[128];
    snprintf(buf, sizeof(buf), "PlayerStart #%d", idx);
    gtk_list_box_insert(GTK_LIST_BOX(ui->obj_scene_list), gtk_label_new(buf),
                        -1);
    gtk_widget_show_all(ui->obj_scene_list);
  }
...
void action_open_object_browser(GSimpleAction *a, GVariant *p,
                                gpointer user_data) {
  (void)a;
  (void)p;
  EditorUi *ui = (EditorUi *)user_data;
  if (!ui)
    return;
  if (ui->obj_browser_win && GTK_IS_WIDGET(ui->obj_browser_win)) {
    gtk_window_present(GTK_WINDOW(ui->obj_browser_win));
    return;
  }
  GtkWidget *win = gtk_window_new(GTK_WINDOW_TOPLEVEL);
  ...
  GtkWidget *zones_list =
      build_template_list(zones, 1, G_CALLBACK(add_zone_template), ui);
  gtk_notebook_append_page(GTK_NOTEBOOK(notebook), zones_list,
                           gtk_label_new("Zones"));
  const char *picks[] = {"Health", "Ammo"};
  GtkWidget *picks_list =
      build_template_list(picks, 2, G_CALLBACK(add_pickup_template), ui);
  gtk_notebook_append_page(GTK_NOTEBOOK(notebook), picks_list,
                           gtk_label_new("Pickups"));
  GtkWidget *assets_box = gtk_box_new(GTK_ORIENTATION_VERTICAL, 4);
  if (ui->last_texture_path)
    gtk_box_pack_start(GTK_BOX(assets_box),
                       gtk_label_new(ui->last_texture_path), FALSE, FALSE, 0);
  if (ui->last_bundle_path)
    gtk_box_pack_start(GTK_BOX(assets_box), gtk_label_new(ui->last_bundle_path),
                       FALSE, FALSE, 0);
  if (ui->last_music_path)
    gtk_box_pack_start(GTK_BOX(assets_box), gtk_label_new(ui->last_music_path),
                       FALSE, FALSE, 0);
  if (!ui->last_texture_path && !ui->last_bundle_path && !ui->last_music_path)
    gtk_box_pack_start(
        GTK_BOX(assets_box),
        gtk_label_new("No assets imported yet. Use File → Import."), FALSE,
        FALSE, 0);
  gtk_notebook_append_page(GTK_NOTEBOOK(notebook), assets_box,
                           gtk_label_new("Assets"));
  const char *nodes[] = {"Player Start"};
  GtkWidget *nodes_list =
      build_template_list(nodes, 1, G_CALLBACK(add_playerstart_template), ui);
  gtk_notebook_append_page(GTK_NOTEBOOK(notebook), nodes_list,
                           gtk_label_new("Nodes"));
  GtkWidget *scene_frame =
      gtk_frame_new("Scene Objects (double-click to edit)");
  GtkWidget *scene_list = gtk_list_box_new();
  for (size_t i = 0; i < ui->obj_count; ++i) {
    const EditorObject *o = &ui->objects[i];
    char buf[128];
    const char *tp = (o->type == OBJ_ZONE
                          ? "Zone"
                          : (o->type == OBJ_PICKUP ? "Pickup" : "PlayerStart"));
    snprintf(buf, sizeof(buf), "%s #%zu", tp, i);
    gtk_list_box_insert(GTK_LIST_BOX(scene_list), gtk_label_new(buf), -1);
  }
  g_signal_connect(scene_list, "row-activated",
                   G_CALLBACK(object_list_row_activated), ui);
...
static void on_toolbar_open(GtkToolButton *btn, gpointer user_data) {
  ...
  if (ag)
    g_action_group_activate_action(ag, "open", NULL);
}
static void on_toolbar_save(GtkToolButton *btn, gpointer user_data) {
  ...
  GActionGroup *ag = G_ACTION_GROUP(g_application_get_default());
  if (ag)
    g_action_group_activate_action(ag, "save", NULL);
}
...
static void build_menu_file(GtkApplication *app, struct EditorUi *ui,
                            GMenu *menubar) {
  (void)app;
  (void)ui;
  ...
  g_menu_append(import_menu, "Import Mesh Bundle (.ozbag)",
                "app.import_bundle");
...
static void build_menu_view(GtkApplication *app, struct EditorUi *ui,
                            GMenu *menubar) {
  (void)app;
  (void)ui;
...
static void build_menu_light(GtkApplication *app, struct EditorUi *ui,
                             GMenu *menubar) {
  (void)app;
  GMenu *light_menu = g_menu_new();
  GMenu *light_add = g_menu_new();
  g_menu_append(light_add, "Add Point Light", "app.light_add_point");
  g_menu_append(light_add, "Add Spotlight", "app.light_add_spot");
  g_menu_append(light_add, "Add Wave Light", "app.light_add_wave");
  g_menu_append_submenu(light_menu, "Add", G_MENU_MODEL(light_add));
  g_object_unref(light_add);
...
static void build_menu_bsp(GtkApplication *app, struct EditorUi *ui,
                           GMenu *menubar) {
  (void)app;
  (void)ui;
  GMenu *bsp_menu = g_menu_new();
  // Brushes
  GMenu *brushes = g_menu_new();
  g_menu_append(brushes, "Box...", "app.bsp_box_dialog");
  g_menu_append(brushes, "Cylinder...", "app.bsp_cyl_dialog");
  g_menu_append_section(bsp_menu, "Brushes", G_MENU_MODEL(brushes));
  g_object_unref(brushes);
  // CSG
  GMenu *csg = g_menu_new();
  g_menu_append(csg, "CSG Add", "app.csg_add");
  g_menu_append(csg, "CSG Subtract", "app.csg_sub");
  g_menu_append(csg, "CSG Intersect", "app.csg_isect");
  g_menu_append_section(bsp_menu, "CSG", G_MENU_MODEL(csg));
  g_object_unref(csg);
  // Tools
  GMenu *tools = g_menu_new();
  g_menu_append(tools, "Carve", "app.tool_carve");
  g_menu_append(tools, "Slope", "app.tool_slope");
  g_menu_append_section(bsp_menu, "Tools", G_MENU_MODEL(tools));
...
void editor_ui_populate_menus(GtkApplication *app, struct EditorUi *ui) {
  const GActionEntry entries[] = {
      ...
      {"light_add_point", on_light_add_point, NULL, NULL, NULL},
      {"light_add_spot", on_light_add_spot, NULL, NULL, NULL},
      {"light_add_wave", on_light_add_wave, NULL, NULL, NULL},sudo 
  ...
  g_menu_append(debug_menu, "JIT: Compile+Run Example",
                "app.debug_jit_example");
