...
OzWatchHandle oz_debug_watch_register(const char *name, const void *address,
                                      OzDebugType type, size_t count);
...
#include "oz_log.h"
static inline void oz_debug_hexdump(const char *label, const void *data,
                                    size_t len) {
  if (!data || len == 0) {
    OZ_DEBUG("%s: (null)", label ? label : "hexdump");
    return;
  }
  ...
  for (size_t i = 0; i < len; i += 16) {
    ...
    for (size_t j = 0; j < 16; ++j) {
      if (j < n)
        r = snprintf(line + o, sizeof(line) - o, "%02x ", p[i + j]);
      else
        r = snprintf(line + o, sizeof(line) - o, "   ");
      if (r < 0)
        r = 0;
      o += (size_t)r;
      if (j == 7) {
...
#define OZ_WATCH_VAR(name, var, type)                                          \
  do {                                                                         \
    static OzWatchHandle _oz_wh = -1;                                          \
    if (_oz_wh < 0)                                                            \
      _oz_wh =                                                                 \
          oz_debug_watch_register((name), (const void *)&(var), (type), 1);    \
  } while (0)
