...
void editor_renderer_set_projection(EditorRenderer* renderer, float fov, float aspect, float near, float far);
...
// Geometry Rendering
void editor_renderer_render_map(EditorRenderer* renderer, const OzMap* map, int selected_index);
void editor_renderer_render_objects(EditorRenderer* renderer, const EditorScene* scene);
void editor_renderer_render_grid(EditorRenderer* renderer, float extent, float step);
void editor_renderer_render_axes(EditorRenderer* renderer, float length);
void editor_renderer_render_gizmo(EditorRenderer* renderer, const OzVec3* center, float scale, int highlight_axis);

// UI and Overlay Rendering
void editor_renderer_render_selection_outline(EditorRenderer* renderer, const EditorScene* scene);
void editor_renderer_render_csg_preview(EditorRenderer* renderer, const EditorScene* scene);
void editor_renderer_render_debug_overlay(EditorRenderer* renderer, const EditorScene* scene);
void editor_renderer_render_performance_info(EditorRenderer* renderer, float fps, float frame_time);

// Lighting Control
void editor_renderer_set_ambient_light(EditorRenderer* renderer, float r, float g, float b);
void editor_renderer_add_point_light(EditorRenderer* renderer, const OzVec3* position, const OzVec3* color, float intensity, float range);
...
void editor_renderer_enable_performance_overlay(EditorRenderer* renderer, bool enabled);
void editor_renderer_capture_frame(EditorRenderer* renderer, const char* filename);

// Utility Functions
void editor_renderer_project_point(EditorRenderer* renderer, const OzVec3* world_pos, float* screen_x, float* screen_y);
void editor_renderer_unproject_point(EditorRenderer* renderer, float screen_x, float screen_y, OzVec3* world_pos);
bool editor_renderer_ray_from_screen(EditorRenderer* renderer, float screen_x, float screen_y, OzVec3* ray_origin, OzVec3* ray_direction);
